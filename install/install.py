...
def color_print(msg, color='red', exits=False):
    ...
    pmsg = color_msg.get(color, 'red') % msg
    print(pmsg)
    if exits:
        time.sleep(2)
        sys.exit()
...
class PreSetup(object):
    ...
    def check_platform(self):
        if not (self._is_redhat or self._is_ubuntu):
            print("支持的平台: CentOS, RedHat, Fedora, Oracle Linux, Debian, Ubuntu, Amazon Linux, 暂不支持其他平台安装.")
            exit()
    ...
    def _input_mysql(self):
        while True:
            mysql = eval(input('是否安装新的MySQL服务器? (y/n) [y]: '))
            if mysql != 'n':
                self._setup_mysql()
            else:
