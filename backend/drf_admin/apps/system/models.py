...
class Permissions(BaseModel):
    ...
    type = models.CharField(
        max_length=8, blank=True, default="", choices=type_choices, verbose_name="权限类型"
    )
...
class Departments(BaseModel):
    ...
    parent = models.ForeignKey(
        "self", null=True, blank=True, on_delete=models.CASCADE, verbose_name="父部门"
    )
    status = models.IntegerField(default=1, verbose_name="状态")
    sort = models.IntegerField(default=0, verbose_name="排序")
    objects = models.Manager()
...
class Users(AbstractUser):
    ...
    mobile = models.CharField(
        max_length=11, unique=True, null=True, blank=True, default=None, verbose_name="手机号码"
    )
    image = models.ImageField(
        upload_to=user_avatar_path, default="avatar/default.png", blank=True, verbose_name="头像"
    )
    roles = models.ManyToManyField(
        Roles, db_table="system_users_to_system_roles", blank=True, verbose_name="角色"
    )
    dept = models.ForeignKey(
        Departments, null=True, blank=True, on_delete=models.SET_NULL, verbose_name="直属部门"
    )
    ...
    def get_menus(self):
        ...
        menus = Permissions.objects.filter(id__in=perm_ids).distinct().order_by("sort")
    ...
    def get_user_info(self):
        role_names = [role.name for role in self.roles.all()]
        user_info = {
            "id": self.pk,
            "name": self.name,
