...
> 默认账户来自开发种子数据，仅用于本地初始化验证。前端登录/注册表单不会在生产或预发环境硬编码预填这些凭据；如需本地演示预填，应在 `frontend/.env.development` 显式配置 `VITE_LOGIN_DEFAULT_USERNAME` 与 `VITE_LOGIN_DEFAULT_PASSWORD`。
...
> 本地开发环境默认数据库通常为 SQLite。当前仓库的 Django 开发库通常位于 `backend/drf_admin/db.sqlite3`，代理在本地调试时可通过 SQLite MCP 连接该文件，但不要把本地 SQLite 的行为直接外推为生产 MySQL 行为。
