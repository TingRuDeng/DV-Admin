...
- **后端 A（Django）**：Django 4.x + DRF + JWT + Channels（WebSocket）
- **后端 B（FastAPI）**：FastAPI + Tortoise ORM（异步，与 Django API 兼容）
...
cd frontend
pnpm install          # 安装依赖
pnpm run dev          # 启动开发服务器（默认端口 9527）
```
...
- [ ] 后端 API 服务正常运行（http://localhost:8769）
- [ ] 数据库连接正常
- [ ] 能够使用默认账户登录
...
1. **API 契约不可随意变更**
   ...
   - 保持 Django 和 FastAPI 两个后端的 API 兼容性
...
1. **两个后端的 API 兼容性**
   - Django 和 FastAPI 后端必须保持 API 接口一致
   - URL 路径、请求参数、响应格式必须相同
