...
| 主题切换的圆形扩散（View Transitions） | 采用（例外） | 从点击位置扩散新主题，坐标只在点击时取一次、不跟随指针；不支持 View Transitions 或开启减少动效时直接切换 |
| 收起侧栏的悬停预览、页签拖拽排序 | 采用 | 都是用户主动触发的交互；悬停预览只是组件内的临时状态，不改写持久化的侧栏展开状态 |
