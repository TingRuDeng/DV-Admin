...
- 普通附件当前只做扩展名、大小、路径归属和文件系统权限校验；不提供深度格式解析、病毒扫描、内容重编码或私有化下载，不能把该上传入口描述为全面文件安全扫描。
- FastAPI 镜像 builder 为 `asyncmy` 准备 Debian 编译工具并从锁文件安装；ARM64 只验证依赖可构建和服务可启动，不宣称跨架构逐字节可复现。
- Django 生产镜像仍以 WSGI 运行；`backend/drf_admin/routing.py` 和 `asgi.py` 已提供 Channels ASGI 契约，但线上切换到 ASGI worker、WebSocket 路由和连接保持仍需部署维护窗口单独验收。
