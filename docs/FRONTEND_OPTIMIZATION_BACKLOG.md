...
> 当前状态：七个串行阶段均已完成；阶段 7 最终选择停止批量壳层迁移并保留现状。后续具体页面问题按独立需求处理，不再视为本路线的自动续期。
...
| 6. 代表页验证 | 已完成（2026-09-02） | 验证用户管理、通知公告、个人中心三个代表页 |
| 7. 扩大或停止评审 | 已完成（2026-09-02） | 决定停止批量壳层迁移并保留阶段 1-6 成果 |
...
- 基准与环境：从阶段 5 合并提交 `f1f649458bae9d4e2b6406be741119c6e28d69a0` 创建独立分支；继续使用 macOS 15.7.7 arm64、Node 24.20.0 和 pnpm 11.21.0。
- 页面与壳层接缝：个人中心为头像上传和资料编辑补齐可访问名称，并适配移动端资料卡与安全设置；用户管理为部门树、数据面板和固定操作列补齐移动端行为；通知公告移除“我的通知”重复发布人列，为详情关闭动作补齐可访问名称，并适配移动端元信息、表格、详情和表单。共享 `DataPanel`、`ProDialog`、`ProDrawer` 只增加移动端视觉接缝，没有改变组件 API。
- 自动化验收：新增个人中心 Mock Playwright，并扩展用户管理、通知公告用例，覆盖筛选、分页、权限按钮、抽屉、发布/撤回、富文本详情、头像、密码校验、静态资源 URL 与 390x844 视口无横向溢出；常规 Mock smoke 扩展为 19 项。Django 与 FastAPI 的隔离测试种子只补齐既有菜单与权限数据，共用无 API Mock 的真实浏览器用例依次验证个人中心、用户管理和通知公告。
- 测试与视觉：`pnpm run quality` 通过（94 个测试文件、285 项测试）；常规 Mock smoke 19/19、Django 真实 smoke 1/1（14.48s）、FastAPI 真实 smoke 1/1（16.79s）均通过。三页 390x844 截图经人工复核，标题、操作区、表格首列、资料卡和通知详情均保持可读且未溢出；临时截图未进入提交。
- 构建对比：按阶段 1 相同口径，以最终生产代码串行运行三次 `/usr/bin/time -p pnpm run build`，结果为 12.69s、11.87s、12.08s，中位数为 **12.08s**；相对阶段 5 的 12.07s 回退约 **0.08%**，相对阶段 1 的 17.53s 改善约 **31.09%**，未触发 10% 停止条件。
- JavaScript 产物：最终构建为 2,692,682 bytes（212 个文件，约 2.57 MiB）；相对阶段 5 增加 84 bytes（约 **0.003%**），相对阶段 1 减少 11,877 bytes（约 **0.44%**），未触发 10% 停止条件。
- 兼容与回滚：继续复用现有 `<script setup>`、Pinia、动态路由、RouteMeta、KeepAlive/cacheKey、字典、WebSocket、SafeHtml 和 ProTable/ProForm 协议；未修改共享 API、菜单字段、组件路径或数据库 schema。完整回退本阶段单一 PR 即可移除页面接缝、测试覆盖和隔离种子，不影响阶段 5 壳层。
- 结论：未触发 ADR-0001 硬停止条件；阶段 6 验收通过。阶段 7 必须在本 PR 合并后基于阶段 1-6 的完整证据独立评审，不默认进入扩大迁移。
...
- 最终决策：选择 **“停止并保留现状”**。保留 Vite 8、Vue Router 5、统一壳层、三种布局、TagsView/KeepAlive、共享页面骨架和三个代表页接缝；不再创建阶段 8，也不启动面向剩余页面的批量迁移。
- 完成证据：阶段 1-6 的前端质量、Mock smoke、Django/FastAPI 真实浏览器、可访问性、性能、人工视觉和回滚检查全部通过。最终构建中位数为 12.08s，较 17.53s 基线改善 31.09%；JavaScript 为 2,692,682 bytes，较基线减少 0.44%；全程未触发任何硬停止条件。
- 复用程度：`frontend/src/views` 的 58 个 Vue 文件中，11 个页面级组件已使用 `PageShell`；9 个后台 CRUD 页面已同时使用 `PageShell`、`ProSearch` 和 request 驱动的 `ProTable`。视图层门禁已禁止旧 CURD 页面组件、非 request 驱动 ProTable 以及原始 `el-dialog` / `el-drawer`，后台业务页面骨架已经闭合。
- 剩余范围：未使用 `PageShell` 的主要是登录、错误、重定向、内部文档、Swagger/Redoc 和 Demo 等特殊承载页。它们不共享后台 CRUD 语义，当前也没有具体视觉或响应式缺陷证据；批量套用同一骨架会增加回归面，却没有可验证的产品收益。
- 成本判断：阶段 5/6 已用独立大范围 diff 建立壳层与代表页证据，继续扩大将重复三布局、移动端、主题、双后端和人工视觉验收成本。dashboard 本身已经使用 `PageShell` 和专属页面样式，不是新的迁移批次。
- 后续边界：若特殊页出现明确缺陷或产品需求，按单页/单问题建立独立 PR 和对应验收，不视为重启本路线；只有出现多个同类正式业务页、可复现问题证据和共享抽象收益时，才通过新 ADR 明确页面清单、兼容边界、性能预算和回滚策略后重新评估批量迁移。
- 独立问题：既有 Lightning CSS `:deep()` 告警、Vite 未来原生配置加载器提示、Vitest/MSW peer 提示和剩余 moderate 依赖公告不属于壳层扩大依据，应在各自治理任务中处理。
- 批次排除：dashboard 已使用 `PageShell` 和专属页面样式，不属于未迁移范围；当前源码没有可由同一后台页面骨架覆盖、且具备明确问题证据的最小扩大批次。

### 2026-09-10 至 2026-09-11 沉浸式首屏与图标治理跟进

**状态：** 本轮实现、质量门禁、Chromium 视觉与交互验收、双后端真实浏览器验收完成；尚未提交、合并或部署。

- 登录页、主壳层、Dashboard、错误页、Demo、内部文档和 Swagger/Redoc 已按各自语义接入暗色画布与荧光珊瑚色视觉层。
- 页面业务图标统一经过 `AppIcon`，依赖迁移到维护中的 `@lucide/vue`；历史 `@element-plus/icons-vue` 导入仅通过隔离适配层兼容，旧本地 SVG 图标目录已删除。
- 已补充图标来源、无表情符号、Dashboard 数据派生、主题和 reduced-motion 治理测试；`pnpm run quality` 为 109 个测试文件、357 项测试通过，生产构建通过。
- 菜单搜索、通知、复制、公告关闭、文件/图片上传、个人中心和退出确认已完成中英文文案与操作标签收敛，上传按钮保留显式 `uploadBtnText` 覆盖能力。
- 修复 Element Plus 函数式图标默认值导致输入框挂载失败、图标选择演示页同名递归、编辑器退出全屏图标状态滞后、用户表格固定列透底等浏览器问题。无行操作权限时隐藏空操作列及批量选择；手机部门筛选默认收起，首页三项指标横排，快捷入口进入首屏。图片预览操作支持键盘聚焦及触屏可见状态。
- 用户表单、导入与公共弹层文案已接入语言包；导入文件通过控件和字段错误限制格式，保留部分成功、全部失败和重试反馈。通知请求失败提供重试，头像缺失或加载失败使用 Lucide 兜底，实心主操作文字随主题色自动选择黑白前景。
- 系统管理页的查询条件、表格列、状态标签、表单校验、确认框和提交反馈统一使用 `system` / `common` 语言包；状态映射在语言切换后即时更新，未改变任何 API、权限或数据字段。已新增语言包键树和系统页面用户可见文案治理测试。
- 六类系统表单的标题、菜单顶级目录和校验消息在语言切换后保持响应；切换时只刷新已经显示的错误状态，不触碰未填写字段。Dict 选择器补齐默认占位文案，英文移动端表单标签通过 390px Chromium 验收，新增 `system-localization.spec.ts` 覆盖桌面与手机尺寸。
- 共享 `TableSelect`、通用 CURD 查询/表单弹层、导入导出弹窗和富文本编辑器的默认操作文案已接入语言包，确认、重置、展开、数据源和上传提示不再依赖中文硬编码。
- 2026-09-13 跟进：修复公告表单缓存的富文本编辑器在语言切换后仍保留旧工具栏与占位文字的问题。同步编辑器原生语言并更新占位缓存，保留实例、草稿、格式和撤销记录；新增浏览器回归覆盖中英文往返及英文初始加载。编辑器及 Vue 适配器加入 Vite 显式预构建列表，减少首次动态加载时补充预构建导致的请求失效。
- 本次跟进验证：先通过浏览器复现英文模式仍显示中文占位文字，修复后公告管理 Chromium 回归 **3/3** 通过（含语言、草稿/格式、撤销重做、权限操作和移动布局）；`pnpm run quality` **111 个文件 / 364 项测试** 通过，生产构建通过，文档、共享 API 和 diff 检查通过。原生语言切换关闭选区，重新聚焦后继续编辑；本次没有重跑双后端真实浏览器门禁。
- 个人中心弹窗标题改为语言键驱动，注册与找回密码表单关闭规则对象变更时的自动校验；个人中心 Chromium 语言切换回归通过，打开资料弹窗后标题更新且草稿保留。
- 最终 Mock Chromium 回归 **30/30** 通过，覆盖登录、三布局、TagsView、动态路由、手机导航、核心业务、搜索键盘操作、图标选择、通知失败重试、图片预览和编辑器全屏往返。登录和查询按钮在明暗主题下通过 4.5:1 文字对比度检查；减少动效设置和主页面无横向溢出检查通过。该记录为覆盖范围内的可访问性验收，不等于全站 WCAG 认证。
- 分别执行隔离的 Django/FastAPI 无 API Mock 浏览器门禁，均 **1/1** 通过（外层 pytest，分别 **41.16s / 43.82s**）；内含共享的个人中心、用户生命周期、RBAC、文件、日志和菜单流程。测试使用临时数据库与隔离 Redis，没有改动已有开发账户或数据库。
- 登录明暗主题 1440px、暗色 390px，以及首页/用户页明暗主题 1440px 和 390px 截图已生成并复核。浏览器启动权限问题已解决。剩余测试日志为动态路由注入前提示、Mock 环境未配置 STOMP，以及预期模拟请求失败；FastAPI 测试进程另有临时密钥配置与 Tortoise 废弃参数提示，不是浏览器验收失败。
- 本分支较早一次性能采样的三次完整构建为 16.24s、14.34s、16.92s，中位数 16.24s；该数据仅为历史采样。最终 `pnpm run build` 通过，`dist/js` 为 **2,501,704 bytes / 160 个文件**；登录性能预算 smoke 通过。没有用单次最终构建冒充新的三次性能中位数。
- 生产依赖审计通过：critical=0、high=0、moderate=3，无生效豁免；文档、共享 API、71 个动态路由组件引用和 diff 格式检查通过。差异复核未发现需要阻止本轮交付的问题，后端 API 与数据库 schema 未变。
- 后续独立产品项：正式注册、找回密码、双后端验证码能力对齐、内部文档真实内容。系统管理域部门、字典、字典项、角色、菜单、日志、通知及我的通知已完成中英文文案接入；内部文档占位外链已移除，当前是空状态；这些产品项不自动扩大本轮壳层重构。
