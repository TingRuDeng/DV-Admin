...
- 两端用户范围使用数据库条件，日志/通知使用单列用户子查询，不把全量可见用户 ID 读入 Python；SELF/部门范围仍按 OR 并集。用户导入只预加载本文件引用的唯一字段、部门和角色，默认每批 500 项，保持原逐行结果和事务。万级测试、查询形态守卫及测量边界见 [PERFORMANCE_BASELINE.md](./PERFORMANCE_BASELINE.md)。
