...
生产构建边界：两端 Python/uv 镜像摘要、应用依赖锁及非 editable 安装已统一；实际产物门禁独立于源码测试。Django Channels ASGI 入口已补齐静态契约，但生产镜像仍运行 WSGI，ASGI worker/WebSocket 线上验收保持开放。
FastAPI ARM64 的 asyncmy 在 builder 中使用 Debian 编译依赖安装，契约测试只证明构建输入明确；尚不宣称逐字节可复现构建。
Django 当前镜像为 WSGI，Channels/WebSocket 的 ASGI 部署需要独立验证。生产媒体卷已统一 `/data/media`，旧媒体复制/权限校验仍须部署维护者执行，不自动迁移；公开媒体 URL 风险保留。

头像已补齐真实格式、完整解码及资源预算，保留既有扩展名兼容面；FastAPI 仍限制既有图片扩展名，Django 保留 Pillow 原有可解码格式。普通附件仍仅扩展名/大小/所有权校验，深度格式检查、病毒扫描、媒体私有化未完成；合法图片的元数据和附加内容不被重编码清除，不能宣称全面上传安全治理已完成。
...
1. 补充 Django 后端测试
2. 完善前端单元测试
3. 继续基于稳定测试数据增加日志清理等核心业务 E2E；涉及共享后端契约的高价值流程优先扩展到双后端真实浏览器矩阵
...
**2026-09-07 查询整改：** 用户数据范围已改为数据库条件，日志/通知使用用户子查询；导入仅预加载本文件引用并按 500 项查询。万级 SQLite 基线与 SQL 形态守卫见 [PERFORMANCE_BASELINE.md](./PERFORMANCE_BASELINE.md)。未新增索引或迁移；MySQL 实际执行计划、组织结构元数据规模、逐行授权/密码哈希、导出全量结果与完整列表字段权限查询仍需独立测量，不将本次定向优化视为全系统性能验收。
...
**具体问题：**
- 批量删除已具备同步结果和失败明细，但尚未纳入可查询的统一任务历史
- 导入导出缺少统一的异步任务进度、历史记录和重试入口

**计划解决方案：**
1. 已完成审计对象关联和结构化请求上下文；后续保持稳定业务命名并为非标准入口显式登记
2. 抽象统一任务状态模型，并把现有批量删除结果接入任务历史
...
- 日志分页已支持按 `requestId` 精确筛选，前端可从响应头或日志详情复制链路标识并定位唯一请求
- 日志已支持 `objectType/objectId` 精确筛选与列表/详情展示；通用 CRUD 默认关联、非标准动作显式登记
- 本轮用户导入继续采用同步 HTTP；双后端通过唯一约束、事务保存点和 `validCount/skippedCount/invalidCount` 结果支持失败重试，不扩展为异步任务历史
- 日志详情已展示结构化 `requestContext`，并沿用后端敏感字段脱敏与大小截断语义
- 用户、角色和通知批量删除已返回统一 `status`/计数/成功项/失败项结果；失败项包含稳定错误码和 `retryable`，可通过专用端点逐条重试
...
- 双后端增加事务内实际权限子集、目标部门解析、多角色并集和关联用户边界检查；默认分配与导入不再绕过角色授权。
- 系统角色保护不可通过改名移除，停用角色不参与授权；API 和敏感字段权限不使用可能陈旧的展示缓存裁决。
- 已补独立 MySQL 8 并发门禁：双后端 × 两种隔离级别 × 六个场景，实际观察锁等待；修复旧快照下角色关联、权限和自定义部门关联未重新读取的问题。入口为 `scripts/verify_mysql_grants.py`，测试库随机生成且运行结束后删除。
- 第二轮已修复同一管理员的角色更新/删除、用户角色调整/批删锁顺序死锁，包含失败项重试入口。门禁扩为双后端 × 两种隔离级别 × 14 个场景，共 56 项；验证两个管理员交叉修改时一个事务失败，以及真实死锁/锁超时在关联写入之后仍整体回滚。
- 收尾定位：面向企业内部小用户量、低并发使用，保留权限一致性与事务回滚保障，既有 56 项 MySQL 门禁不再扩大。同步导入结果已补齐成功/跳过/失败语义；复杂授权交错、菜单/部门拓扑并发、导入等长事务和负载优化，仅在实际出现冲突、锁等待或性能问题时另行评估，不作为本轮完成条件；异步任务与性能专项不纳入本轮。
- 剩余风险：多管理员交叉编辑仍可能死锁；允许罕见冲突明确失败并完整回滚，失败项不得计为成功，不自动重试、不承诺所有组合无死锁。角色删除为保持原锁内复核，额外前置一次操作者检查；未做查询数量优化或全局锁表。Django 既有数据库异常 HTTP 507 与 FastAPI 异常映射差异未纳入本轮。非目标仍包括菜单/部门对象治理、私有附件与 iframe 隔离。
...
- 用户、操作日志和后台通知的列表及对象级读写路径已强制应用数据范围过滤；范围外对象按不存在处理，用户/角色/通知初次批量删除先整批预检，预检通过后允许逐条失败并返回结果
...
**描述：** 非安全缓存可以使用内存后备；FastAPI 生产认证撤销状态不得降级。
...
**2026-09-07 安全边界修正：** FastAPI 生产撤销状态读写和刷新消费失败返回 503；刷新保留原会话时间并检查用户级撤销。两端生产就绪探针检查数据库和 Redis 并真实返回 503，Nginx 不再固定返回健康。实现及验证在 `tasks/audit-remediation-2026-09-07.md` 跟踪，不能将进程内存模式当作多实例撤销存储。

**登录防刷：** 两端登录使用 Redis 原子账号冷却与 IP 限速，不采用进程内缓存；429 保留 `Retry-After`，Redis 不可用返回 503。代理地址必须显式配置，默认忽略转发头。剩余风险包括共享公网出口被整体限速、攻击者持续触发账号短时冷却；不宣称等价于 MFA 或彻底消除撞库。

**2026-09-07 新密码边界：** 两端新密码统一为默认 15-128 个 Unicode 字符和固定版本常见密码拒绝清单；FastAPI 新哈希转为 Django 兼容 PBKDF2-SHA256 600,000 轮，旧 bcrypt/PBKDF2 只保留验证兼容。创建/导入/重置使用的部署初始密码必须通过策略，不自动更换。旧弱密码、已泄露但不在本地清单中的密码、共享初始密码分发仍是剩余风险；本轮不引入激活流程或在线泄露密码查询。
