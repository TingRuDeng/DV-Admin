...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限；数据范围已覆盖角色配置、用户列表、操作日志列表和后台通知管理，字段读取已覆盖用户、操作日志敏感字段和通知 `targetUserIds`，字段写入第三阶段已覆盖后台用户管理的 `mobile/email` 写入拒绝，字段写入第四阶段已覆盖后台通知的 `targetUserIds` 写入拒绝，但尚未覆盖所有业务对象。
...
- 用户、操作日志和通知目标用户字段已具备字段读取脱敏；通知内容和其他对象尚未接入字段读取策略。
...
- 用户、操作日志和通知目标用户字段已支持细粒度原文权限；其他对象仍只能依赖接口整体权限。
...
2. 继续评估通知内容和其他剩余业务对象是否需要接入字段读取脱敏。
