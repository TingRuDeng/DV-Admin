...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限，但尚未覆盖数据范围和字段级读写控制。用户只要拥有接口权限，就能读取该接口返回的全部授权数据与字段。

**具体问题：**
- Django `RBACPermission` 与 FastAPI `PermissionChecker` 只做接口级权限校验。
- 角色表单类型中 `dataScope` 仍是注释字段，前后端均未形成真实契约。
- 用户、日志、通知等高敏感对象缺少字段读取隐藏、脱敏和写入拒绝策略。

**影响范围：**
- 多部门、多角色场景下无法表达“只能看本部门/本人/自定义部门”的产品规则。
...
**计划解决方案：**
1. 先确认产品语义，尤其是本人数据、部门树、自定义部门和多角色合并规则。
2. 第一阶段只做数据范围最小闭环，并补双后端越权测试。
3. 第二、三阶段再接入字段读取隐藏/脱敏和字段写入拒绝。
