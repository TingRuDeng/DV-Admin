...
**描述：**
前端核心请求、存储、路由、标签页、设置和 WebSocket 类型边界已经分阶段收口。首批系统 API 类型字段已通过 `scripts/api_frontend_field_contracts.py` 挂靠后端字段契约，但演示页面和部分历史组件仍需持续治理。

**具体问题：**
- 前端 API 类型目前只覆盖用户、角色、菜单、部门、字典、字典项和通知等高价值对象
...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限；数据范围已覆盖角色配置、用户列表、操作日志列表和后台通知管理，字段读取已覆盖用户、操作日志敏感字段、通知 `targetUserIds` 和后台通知正文 `content`，字段写入第三阶段已覆盖后台用户管理的 `mobile/email` 写入拒绝，字段写入第四阶段已覆盖后台通知的 `targetUserIds` 写入拒绝，但尚未覆盖所有业务对象。

**具体问题：**
- Django `RBACPermission` 与 FastAPI `PermissionChecker` 仍只做接口级权限校验；数据范围过滤由用户/日志列表服务单独接入。
- 角色表单类型、双后端角色 schema 和契约目录已形成 `dataScope/deptIds` 最小闭环。
- 用户、操作日志、通知目标用户字段和后台通知正文已具备字段读取脱敏；其他对象尚未接入字段读取策略。
- 个人中心资料维护等其他对象尚未接入字段写入拒绝策略。

**影响范围：**
- 用户列表、操作日志列表和后台通知管理已支持“只能看本部门/本人/自定义部门”的数据范围规则。
- 后台通知管理按通知 `publisher_id` 过滤列表和按 ID 管理动作；FastAPI 表单查询也使用同一规则。“我的通知”仍按接收人可见性判断。
- 用户、操作日志、通知目标用户字段和后台通知正文已支持细粒度原文权限；其他对象仍只能依赖接口整体权限。
- 后续补能力时需要同时改 Django、FastAPI、前端、契约和迁移，属于跨模块高风险变更。

**当前治理进展：**
- 已新增 `tasks/data-field-permission-prd.md`，明确数据范围枚举、字段权限语义、双后端实施边界和分阶段验收标准。
- 已新增角色级 `dataScope/deptIds`，并在 Django 与 FastAPI 的用户列表、操作日志列表和后台通知管理强制应用数据范围过滤。
- 已补双后端越权回归测试，防止用户/日志列表退回全量查询。
- 已新增 `system:users:field:plain` 与 `system:logs:field:plain` 字段原文权限；没有原文权限时，用户 `mobile/email` 和日志 `requestBody/responseBody/ip` 在 Django/FastAPI 输出中保留字段但返回脱敏值。
- 已新增 `system:notices:target:plain` 字段原文权限；没有原文权限时，通知 `targetUserIds` 在 Django/FastAPI 输出中保留字段但返回空数组。
- 已新增 `system:notices:content:plain` 字段原文权限；没有原文权限时，后台通知管理 `content` 在 Django/FastAPI 输出中保留字段但返回 `[已脱敏]`，“我的通知”正文保持可读。
- 已新增 `system:users:field:write` 字段写入权限；后台用户创建/更新请求中显式写入非空 `mobile/email` 时，Django/FastAPI 均会要求该权限或 `is_superuser`。
- 已新增 `system:notices:target:write` 字段写入权限；后台通知创建/更新请求中显式写入非空 `targetUserIds` 时，Django/FastAPI 均会要求该权限或 `is_superuser`。
- 已新增字段权限码目录守卫，要求上述字段权限同步存在于运行时代码、Django 初始权限树和 FastAPI 测试权限 fixture，避免权限校验可执行但后台无法授予。
- 已新增后台通知管理数据范围过滤；Django/FastAPI 均按 `publisher_id` 限制列表和按 ID 管理动作，FastAPI 表单查询也使用同一规则。

**计划解决方案：**
1. 继续评估除用户、日志和后台通知外的剩余业务对象是否需要接入数据范围过滤。
2. 继续评估其他剩余业务对象是否需要接入字段读取脱敏。
3. 继续评估个人中心资料维护等剩余业务对象是否需要接入字段写入拒绝。
...
**计划解决方案：**
1. 先定义审计日志与批量任务 PRD：`tasks/audit-batch-import-export-prd.md`
2. 抽象统一任务状态模型
