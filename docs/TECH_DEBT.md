...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限；数据范围第一阶段已覆盖角色配置、用户列表和操作日志列表，字段读取第二阶段已覆盖用户与操作日志敏感字段脱敏，字段写入第三阶段已覆盖后台用户管理的 `mobile/email` 写入拒绝，字段写入第四阶段已覆盖后台通知的 `targetUserIds` 写入拒绝，但尚未覆盖所有业务对象。
...
**影响范围：**
- 用户列表和操作日志列表已支持“只能看本部门/本人/自定义部门”的第一阶段规则。
- 通知等其他对象仍未接入数据范围过滤。
...
- 已新增角色级 `dataScope/deptIds`，并在 Django 与 FastAPI 的用户列表、操作日志列表强制应用数据范围过滤。
