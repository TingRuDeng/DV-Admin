...
- 前端：已补 store/router、WebSocket、API 契约、可访问性和性能 smoke，以及核心管理业务 Mock E2E；另有一份无 API Mock 的 Playwright 流程在 CI 中分别连接 Django/FastAPI，覆盖登录、资料、头像、通知，以及角色授权/撤权后的动态菜单、按钮和接口权限闭环。跨业务完整真实栈 E2E 仍不充分
...
1. 补充 Django 后端测试
2. 完善前端单元测试
3. 继续基于稳定测试数据增加菜单写入、文件上传/删除等核心业务 E2E；涉及共享后端契约的高价值流程优先扩展到双后端真实浏览器矩阵
