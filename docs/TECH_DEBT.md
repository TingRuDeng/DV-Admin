...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限；数据范围已覆盖角色配置、用户列表、操作日志列表和后台通知管理，字段读取第二阶段已覆盖用户与操作日志敏感字段脱敏，字段写入第三阶段已覆盖后台用户管理的 `mobile/email` 写入拒绝，字段写入第四阶段已覆盖后台通知的 `targetUserIds` 写入拒绝，但尚未覆盖所有业务对象。
...
- 用户与操作日志已具备字段读取脱敏；通知等其他对象尚未接入字段读取策略。
...
- 用户和操作日志高敏感字段已支持细粒度原文权限；其他对象仍只能依赖接口整体权限。
