...
**描述：**
部分历史 Vue 单文件组件超过 300 行，承载了页面编排、弹窗表单、列表操作和兼容逻辑等多重职责。

**具体问题：**
- `frontend/src/components/CURD/PageContent.vue` 仍是历史兼容层，文件体量较大
- 系统管理、通知、角色、用户、设置等页面存在表单弹窗和列表逻辑耦合
- 大文件会增加后续类型治理、交互调整和回归测试成本
...
**影响范围：**
- 前端页面维护和局部重构成本偏高
- 大范围修改时更容易引入样式、状态或权限行为回归

**计划解决方案：**
1. 按页面优先级拆分表单弹窗、筛选区、列表操作和详情展示
2. 保留现有 API 契约与 UI 行为，逐页补最小回归测试
3. 对历史兼容组件先标注边界，避免新页面继续扩展旧抽象

**预计工作量：** 分阶段持续治理

---

### 10. FastAPI 默认密码配置仍需收口

**级别：** 🟢 低

**描述：**
FastAPI 后端仍通过独立的 `DEFAULT_PASSWORD` 配置控制新增、导入和重置用户的默认密码，当前样例文件仍保留弱默认值。

**具体问题：**
- `fastapi/.env.example` 中 `DEFAULT_PASSWORD=123456`
- `fastapi/app/core/config.py` 为未显式配置时提供 `Admin@123456` 默认值
- `fastapi/app/api/v1/system/users.py` 的接口说明仍需避免暗示固定弱口令

**影响范围：**
- 新开发者按样例配置 FastAPI 时可能继续使用弱默认密码
- Django 已收口 `DEFAULT_PWD`，两套替代后端在默认密码治理上仍不完全一致

**计划解决方案：**
1. 将 FastAPI `DEFAULT_PASSWORD` 改为必须显式配置
2. 更新 `fastapi/.env.example` 为非真实占位值
3. 补充 FastAPI settings 回归测试，防止弱默认值回归
