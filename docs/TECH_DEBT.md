...
**描述：**
FastAPI 已提供 `OperationLog` 模型和 `/api/v1/system/logs/*` 查询、统计、删除接口；Django 当前只有 `OperationLogMiddleware` 写文件日志，没有等价数据库审计日志模型或可查询 API。

**具体问题：**
- 前端日志管理页当前只能依赖 FastAPI 后端。
- 双后端 API 契约在操作日志域存在能力漂移。
- Django 文件日志可用于排障，但不能替代产品层审计查询能力。
...
**计划解决方案：**
1. 单独评估是否补 Django `OperationLog` 模型、迁移、路由和权限。
2. 前端日志管理页已在日志分页接口返回 404/405 时显式提示当前后端未提供可查询操作日志能力，并用治理测试锁定该边界。
3. `scripts/api_capability_contracts.py` 已将 `/api/v1/system/logs/*` 登记为 FastAPI 独占能力，并由 API 契约校验器检查 FastAPI 路由证据和 Django 路由缺席证据。
4. 后续仍需决策是否补 Django 实现；如不补 Django 实现，则继续在产品层标记操作日志为 FastAPI 独占能力。
...
**描述：**
`scripts/api_field_contracts.py` 已冻结首批 Django/FastAPI 响应字段漂移，并用 `converge` 标记需要后续清零的差异。当前测试转绿只表示漂移已被登记和监控，不表示双后端行为已经等价。

**具体问题：**
- 用户和角色权限字段仍存在 `roleNames/roles/rolesList/permissions` 差异，涉及 `users_out.roleNames`、`users_out.roles`、`users_out.rolesList`、`users_form_out.roleNames`、`users_form_out.rolesList`、`roles_out.permissions`
- 菜单字段仍存在父级、children 和时间戳差异，涉及 `menus_out.createTime`、`menus_out.createdAt`、`menus_out.parent`、`menus_out.parentId`、`menus_out.updateTime`、`menus_out.updatedAt`、`menus_tree.children`、`menus_tree.label`、`menus_tree.parent`、`menus_tree.parentId`
- 字典字段仍存在时间戳差异，涉及 `dicts_out.createTime`、`dicts_out.createdAt`、`dicts_out.updateTime`、`dicts_out.updatedAt`
- 部门字段仍存在时间戳、children 和父级名称差异，涉及 `depts_out.createdAt`、`depts_out.updatedAt`、`depts_tree.children`、`depts_tree.parentName`
- 通知分页字段仍存在目标用户和更新时间差异，涉及 `notices_page.targetUserIds`、`notices_page.updateTime`

**影响范围：**
- FastAPI 下字典标签配色、Django 下用户角色列等用户可见行为仍可能不等价
- 新增或修改读端点时，如果未同步字段契约，前端切换后端仍可能出现隐性字段缺失

**计划解决方案：**
1. 逐项决定字段收敛方向：补字段、改前端兼容读取，或产品层声明后端独占
2. 每修复一组差异，同步从 `converge` 与本技术债条目删除
3. 保留读端点字段契约覆盖校验，防止新增共享读端点绕过字段契约
