...
- 前端：已补 store/router、WebSocket、API 契约、可访问性和性能 smoke，以及核心管理业务 Mock E2E；另有一份无 API Mock 的 Playwright 流程在 CI 中分别连接 Django/FastAPI，覆盖登录、资料、头像、通知、角色授权/撤权、菜单与子权限写入，以及通用文件上传、所有权校验和删除闭环。跨业务完整真实栈 E2E 仍不充分
...
1. 补充 Django 后端测试
2. 完善前端单元测试
3. 继续基于稳定测试数据增加用户写入、密码重置和日志清理等核心业务 E2E；涉及共享后端契约的高价值流程优先扩展到双后端真实浏览器矩阵
...
**具体问题：**
- 审计日志已持久化 `requestId`、支持按请求 ID 精确筛选并采集失败响应摘要，但尚未记录对象类型和对象 ID
- 批量操作缺少统一任务状态和失败明细反馈
