...
**描述：**
当前 RBAC 已覆盖接口、菜单和按钮权限；数据范围第一阶段已覆盖角色配置、用户列表和操作日志列表，但尚未覆盖字段级读写控制以及所有业务对象。
...
- 用户、日志、通知等高敏感对象缺少字段读取隐藏、脱敏和写入拒绝策略。

**影响范围：**
- 用户列表和操作日志列表已支持“只能看本部门/本人/自定义部门”的第一阶段规则。
- 通知等其他对象仍未接入数据范围过滤。
- 高敏感字段只能依赖接口整体权限，无法做细粒度暴露控制。
