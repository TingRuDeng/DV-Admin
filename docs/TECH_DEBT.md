...
**描述：**
`scripts/api_field_contracts.py` 已将首批 Django/FastAPI 响应字段漂移显式登记。当前 `converge` 收敛清单已清零，剩余差异主要是单端扩展字段，需要逐项判断是继续保留、补齐到共享契约，还是从输出中删除。

**具体问题：**
- 用户、角色、菜单、字典、部门等对象仍存在若干 `django_only` 或 `fastapi_only` 扩展字段。
- `rolesList`、`dateJoined`、`isSuperuser`、`avatar`、`gender` 等字段需要按前端读取点和产品语义复核是否继续保留。

**影响范围：**
- 新增或修改共享端点时，如果未同步字段契约，前端切换后端仍可能出现隐性字段缺失。
- 单端扩展字段长期保留会增加 API 类型、文档和测试理解成本。

**计划解决方案：**
1. 逐项复核单端扩展字段：保留为扩展、补齐到共享契约，或从输出中删除。
2. 每调整一组字段，同步更新 `scripts/api_field_contracts.py`、前端字段契约和相关文档。
