...
用户导入返回 `validCount`、`skippedCount`、`invalidCount` 和 `messageList`；有效行可能已入库，重复提交行属于幂等跳过，不能仅因存在失败行就提示“上传失败”或跳过列表刷新。

- 全部成功：成功提示并关闭导入抽屉；部分成功：警告提示成功/失败数量并保留失败明细；全部失败：错误提示并保留失败明细。
- 只要 `validCount > 0` 就通知用户列表刷新；零条记录不提示成功、不刷新。请求异常不伪造导入数量。
- 开始新的提交时清除上一次结果，避免异常后旧明细被误认为本次结果；查看或关闭明细不重复触发刷新。
- 实现及回归：`frontend/src/views/system/user/components/UserImport.vue`、同目录 `__tests__/user-import.spec.ts`。这是同步导入的反馈修复，不引入异步任务、任务历史或自动重试。
- 浏览器回归：`frontend/e2e/user-management.spec.ts` 使用真实 Chromium 和隔离 Mock API 验证部分成功后的列表刷新、失败明细保留及再次提交异常后的旧结果清理；不作为真实后端入库验证。
...
### scoped SCSS 不能只靠源码字符串验收

- Vue scoped 样式中的 `:global(.mobile)` 与嵌套后代选择器组合后，编译结果可能只保留 `.mobile`，导致抽屉定位规则未命中侧栏。
- mix 布局使用普通 `.mobile` 祖先选择器，让 Vue 将作用域标记附加在后代元素上。
- 回归验证同时检查 Sass + Vue 编译后的完整选择器，以及移动端浏览器中的实际定位和抽屉交互；源码中出现选择器不等于样式生效。

### JWT 签发时间精度必须与撤销时间兼容

- python-jose 会将 datetime 类型的 `iat` 截断为整秒，而用户令牌撤销时间包含微秒，可能误拒绝同秒撤销后重新登录签发的令牌。
- FastAPI 使用带小数的 NumericDate 签发 access/refresh token，保留真实签发顺序；不能通过截断撤销时间放行同秒旧令牌。
- 修改密码必须先成功撤销旧令牌，再更新密码；验证既要确认旧令牌返回 401，也要确认新密码重新登录后可以访问受保护接口。
