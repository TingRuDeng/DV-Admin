...
class DependencyAuditTests(unittest.TestCase):
    ...
    def test_timeout_still_fails_strictly(self, run_process):
    ...
    @patch("scripts.validate_dependency_audit.subprocess.run")
    def test_timeout_is_strict_validation_error(self, run_process):
        run_process.side_effect = subprocess.TimeoutExpired(["pnpm", "audit"], 120)

        with self.assertRaises(AuditValidationError) as context:
            run_pnpm_audit()

        self.assertIs(type(context.exception), AuditValidationError)
