...
const PAGE_MODAL_MAPS_SOURCE = readFileSync(
  resolve(CURD_ROOT, "pageModalComponentMaps.ts"),
  "utf8"
);

describe("CURD component map type governance", () => {
  it("keeps component maps away from explicit any", () => {
    const sources = [PAGE_SEARCH_SOURCE, PAGE_MODAL_SOURCE, PAGE_MODAL_MAPS_SOURCE].join("\n");
  ...
  it("keeps PageModal below the file size hard limit", () => {
    expect(PAGE_MODAL_SOURCE.split("\n").length).toBeLessThan(300);
  });
});
