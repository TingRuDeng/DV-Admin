...
PIC_URL = config.get('base', 'pic_url')
...
# ====== alert ======
AlertSource = config.get('alert', 'alert_source')
AlertReceiver = config.get('alert', 'alert_receiver')
...
# ------ celery
BROKER_URL = 'redis://:%(password)s@%(host)s:%(port)s/4' % {
        'password': RedisPassword,
        'host': RedisServer,
        'port': RedisPort,
    }
CELERY_RESULT_BACKEND = BROKER_URL
# CELERY_IMPORTS = ('jcoin.tasks')
