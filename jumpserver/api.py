...
def send_emails(mail_obj, to_email, cc_email, msg, picture_name=None):
    subject = mail_obj.subject
    plain_text = get_template(mail_obj.template_path)
    # if mail_obj.email_host_user:
    #     conn = get_connection()
    #     conn.username = mail_obj.email_host_user
    #
    #     from_email = mail_obj.email_host_user
    # else:
    from_email = EMAIL_HOST_USER
    text_content = plain_text.render(Context(msg))

    if cc_email:
        msg = EmailMessage(subject, text_content, from_email, to_email, cc=cc_email)
    else:
        msg = EmailMessage(subject, text_content, from_email, to_email)
    if picture_name:
        image = add_img(picture_name)
        msg.attach(image)
...
#        os.chmod(log_file, 0o777)
    ...
    ah = AlertHandler()
    ah.setLevel(logging.CRITICAL)
    logger_f.addHandler(ah)
    return logger_f
...
def att_pages(post_objects, request):
    """
    page public function , return page's object tuple
    分页公用函数，返回分页的对象元组
    """
...
def filter_object(request, object, values):
    """
    根据用户角色对数据进行过滤
    :param object: 
    :return: 
    """
    if request.user.code == 'CU':
...
#     if request.user.role.code == role_all.get(role, 'CU'):
#         return True
#     else:
#         return False
...
def defend_attack(func):
    def _deco(request, *args, **kwargs):
        if int(request.session.get('visit', 1)) > 15:
...
def date_to_time(date):
    ...
    # s = datetime.time.strftime(d, '%H:%M:%S')
    return d


def get_top_dep(user_obj):
    """
    获取员工的一级部门
    :param user_id: 
    :return: 
    """
    try:
        dep_obj = user_obj.group
        if dep_obj:
            if dep_obj.parent:
                return dep_obj.parent.id
            else:
                return dep_obj.id
        return None
    except Exception as e:
        logger.error('获取员工一级部门异常：%s' % e)
        return None


# def is_error_request(object):
#     print "==",object
#     if object == []:
#         message = u'权限不足'
#         return my_render('error.html', locals(), request=message)
#     else:
#         print "2==e",object
#         pass


def format_to_time(d_time):
    """
    格式化utc时间
    :param d_time: 
    :return: 
    ...
    d_date = datetime.datetime.strptime(
        d_list[0],
        '%Y-%m-%dT%H:%M:%S'
    ) + datetime.timedelta(hours=(8-int(d_list[1].split(':')[0])))
    return d_date
