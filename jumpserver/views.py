...
# from jcoin.coin_api import cal_amount, apply_user_coin, get_top_user_value, get_macth_bid_status, get_top_user_win
# from jcoin.models import Bid, Match
from juser.models import Game, GameContent
...
def index(request):
    ...
    if request.user.role.code in ['GA', 'SU']:
    ...
    if request.user.role.code == 'SU' or request.user.role.id == 6:
        ...
        # login_user = get_recent_login(7)
        active_users = User.objects.filter(is_active=1)
        # active_hosts = Assets.objects.filter(is_active=1)
        ...
        # amount = cal_amount(get_object(User, id=request.user.id))
        # 最后10次权限申请
        # perm apply latest 10
        # perm_apply_10 = Apply.objects.order_by('-date_add')[:10]

        # 预近期入职人员（一周）
        # prepare_user_list = get_recent_join_user(7)

        # 近期入职人员（一周）
        # entry_user_list = get_recent_entry_user(7)

        # 近期离职人员（一周）
        # departure_user_list = get_recent_leave_user(7)

        # 近一个月采购记录
        # month_buy_list = get_recent_buy_asset(30)

        # 最近10条资产变更记录
        # asset_change_logs = get_recent_change_log(10)
...
def user_index(request):
    header_title, path1, path2 = '首页', '信息查询', '游戏信息列表'
    ...
    try:
        ...
        #     m_obj = MailContentTmp.objects.filter(create_time__range=(s_time, e_time))
        # elif s_time:
        #     m_obj = MailContentTmp.objects.filter(create_time__gte=s_time)
        # elif e_time:
        #     m_obj = MailContentTmp.objects.filter(create_time__lte=e_time)
        # else:
        #     m_obj = MailContentTmp.objects.all()
        # if title:
        #     m_obj = m_obj.filter(title__icontains=title)

        game_obj = Game.objects.all()

        # if keyword:
        #     game_obj = game_obj.filter(content__icontains=keyword)

        contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(game_obj, request)
        return my_render('juser/content_list.html', locals(), request)
    except Exception as err:
        logger.error('user get index html error::%s' % err)


@require_role('user')
def mail_info(request):
    g_id = request.GET.get('id', '')
    m_obj = GameContent.objects.filter(game_id=g_id)
    contact_list, p, contacts, page_range, current_page, show_first, show_end = pages(m_obj, request)
    return my_render('juser/game_content_info.html', locals(), request)
...
def Login(request):
    ...
    if request.method == 'GET':
        ...
        if username and password:
            ...
            if user is not None:
                # if user.is_active and (user.role.id in (2, 3)):
                login(request, user)
                logger.warn('user %s login system' % username)
                # if not user.coin_addr:
                    # apply_user_coin(user)
                if user.role is None:
                    return render_to_response('login.html')
                if user.role.code == 'SU':
                    request.session['role_id'] = 2
                elif user.role.code == 'CU':
...
#             asset_select.append(get_object(Assets, id=asset_id))
#
...
#     asset = get_object(Assets, id=asset_id)
#     if asset:
