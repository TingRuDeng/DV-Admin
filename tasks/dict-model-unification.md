...
已有 FastAPI 数据库如果仍存在旧表 `system_dict_data`、`system_roles_permissions`、`system_users_roles`，仍存在旧关联字段 `user_id/role_id/permission_id`，或仍存在旧字典项扩展字段 `is_default/remark`，需要通过显式数据库迁移切换到目标表、目标字段或删除旧列；业务代码不提供双表、双字段或扩展字段静默 fallback。
...
- [x] P12 串行：将 FastAPI 角色-权限和用户-角色关联字段统一到 Django 命名。
- [x] P13 串行：同步共享关联字段契约、数据库文档、技术债和字典治理计划。
- [x] P14 串行：移除 FastAPI 字典项 `is_default/remark` 扩展字段。
- [x] P15 串行：同步数据库文档、技术债和字典治理计划。
