...
- [x] P4 串行：执行模型契约校验、FastAPI 目标测试、FastAPI 质量门禁、Django 目标测试和根目录校验
- [x] P5 串行：review-gate、提交、PR、CI 和合并
- [ ] P1 串行：RED 补共享关联表同名测试，复现角色-权限和用户-角色 through 表仍不一致
- [ ] P2 串行：GREEN 修改 FastAPI 两张关联表名和共享关联契约
- [ ] P3 串行：执行并修正 FastAPI M2M 目标测试
- [ ] P4 串行：同步数据库文档、技术债和字典模型治理计划
- [ ] P5 串行：执行模型契约校验、FastAPI 目标测试、FastAPI 质量门禁、Django 目标测试和根目录校验
