...
- [x] P1 串行：RED 新增通知公告权限 E2E，复现后端 `system:notices:*` 权限下前端写按钮不可见
- [x] P2 串行：将通知公告前端按钮权限码从 `sys:notice:*` 对齐为 `system:notices:*`
- [x] P3 串行：将通知公告 E2E 纳入 smoke 脚本，并用治理测试锁定
- [x] P4 串行：执行目标 E2E、完整 smoke、前端质量和根目录必要校验
- [ ] P5 串行：同步任务状态、review-gate、提交、PR、CI 和合并

并行判断：本轮涉及通知公告 E2E、通知页模板权限码、smoke 脚本和治理测试，存在明确顺序依赖；为保证 RED/GREEN 证据清晰，采用串行推进，不启用 subagent。
