...
- [x] P1 串行：补动态路由组件缺失的前端失败测试
- [x] P2 串行：补后端菜单种子组件路径与前端视图文件的契约校验
- [x] P3 串行：修复动态路由解析，禁止组件缺失时静默落到 404
- [x] P4 串行：修正 FastAPI 测试权限种子的组件路径漂移
- [x] P5 串行：执行前端目标测试、契约校验、文档校验与 review gate

并行判断：本轮会同时触达前端路由 store、前端单测、契约校验脚本、CI 配置和 FastAPI 测试权限种子；这些文件围绕同一动态路由契约，存在顺序依赖和验证闭环，为避免局部补丁互相遮蔽，采用串行推进。
